...
public class AuthenticationManager implements UserDetailsService {
	private static final Log log = LogFactory
			.getLog(AuthenticationManager.class);
	...
	/**
	 * @deprecated the preferred way to get the default manager is using Spring
	 *             injection
	 */
	public static AuthenticationManager getDefault() {
		return (AuthenticationManager) SpringUtils
				.getSpringBean("userDetailsService");
	}
	 ...
	 *            delegate for getting user's roles
	 */
	public AuthenticationManager(ConfigurationUtil cfg,
	...
	public Principal getCurrentPrincipal() {
		Object ret = SecurityContextHolder.getContext().getAuthentication()
				.getPrincipal();
	...
	public UserDetails loadUserByUsername(String username)
		...
		try {
			...
			if (db.compareTo(code, Version.MINOR) == 0) {
				...
				log.info("loadUserByUsername - getting user " + username
						+ " using JDBC");
			...
			throw new DataRetrievalFailureException(
					"Error getting roles for user: " + username, e);
		} catch (DataIntegrityException e) {
			log.warn("loadUserByUsername - exception", e);
			throw new DataIntegrityViolationException(
					"Inconsistent user name: " + username, e);
			...
			log.warn("loadUserByUsername - exception", e);
			throw new DataRetrievalFailureException("Error getting user: "
					+ username, e);
	 ...
	 */
	public String resetPassword(User user, String[] rnd0, String[] rnd1,
			String[] rnd2, String[] rnd3, String[] rnd4) {
		String changedPassword = generateRandomPassword(rnd0, rnd1, rnd2, rnd3,
				rnd4);
	 ...
	 */
	private String generateRandomPassword(String[] rnd0, String[] rnd1,
			String[] rnd2, String[] rnd3, String[] rnd4) {
	 ...
	 */
	private Principal jdbcSearchByLogin(String login) throws DataAccException,
			DataNotFoundException, DataIntegrityException {
		...
		try {
			...
			try {
				stmt = con
						.prepareStatement("select u.id,u.login,u.password,u.active,u.name,r.id roleId "
								+ "from User u,Role r "
								+ "where u.roleId=r.id " + "and u.login=?");
				...
				if (rs.next()) {
					...
					ret = new Principal(
							rs.getInt("id"),
							0, // In console mode we won't need departmentId so
					...
					if (rs.next()) {
						throw new DataIntegrityException(User.class,
								"Duplicated user login: " + login);
					}
				} else {
