...
public class UserManager {
    ...
    public void updateEntity(User user, boolean agreementYearDuration) {
        this.doBeforeSave(user, agreementYearDuration);
        User oldUser = userDAO.loadById(user.getId());
        final var principalName = getPrincipalName(oldUser);
        final var newPrincipalName = getPrincipalName(user);
        userDAO.update(user);
        archimedesSecuritySubjectDAO.findByPrincipalName(principalName).ifPresent(subject -> {
            subject.setPrincipalName(newPrincipalName);
            archimedesSecuritySubjectDAO.update(subject);
        });
    }
    ...
    public void deleteEntity(User user) {
        userDAO.delete(user);
        final var principalName = getPrincipalName(user);
        archimedesSecuritySubjectDAO.findByPrincipalName(principalName).ifPresent(subject -> {
            archimedesSecuritySubjectDAO.delete(subject);
        });
    }
    ...
    private String getPrincipalName(final User user) {
        return user.getLogin() + "@autentia.com";
    }
    ...
    private ArchimedesSecuritySubject createArchimedesSecuritySubjectFromUser(final User user) {
        ...
        return ArchimedesSecuritySubject.from(principalName, subject);
    }
}
