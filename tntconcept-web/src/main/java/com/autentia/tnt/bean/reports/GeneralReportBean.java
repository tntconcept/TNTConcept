...
public class GeneralReportBean extends BaseBean {
	...
	private static final Log log = LogFactory.getLog(GeneralReportBean.class);
	...
	public void setSelectedReport(String selectedReport) {
		...
		int numReports = 0;
		numReports = ReportManager.getReportManager().getReportListGeneral().size();
		for(int i=0;i<numReports;i++) {
			List reportData = new ArrayList<List>();
			String reportName = "";
			reportData = ReportManager.getReportManager().getReportListGeneral().get(i);
			...
			if(reportName.equals(selectedReport)) {
				...
				ReportUtil.createReportParameterDefinition(parameters, reportParametersDefinitions,getUsers(),getOrgs(),null,null);
