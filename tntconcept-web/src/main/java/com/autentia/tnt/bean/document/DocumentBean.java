...
public class DocumentBean extends BaseBean {
	...
	public List<SelectItem> getVersionss() {
		List<DocumentVersion> refs = DocumentVersionManager.getDefault().getAllEntities(null,
				new SortCriteria(ROW_ID));
	...
	public String detail() {
		...
		return SpringUtils.isAclPermissionGranted(document, BasePermission.WRITE) ? NavigationResults.EDIT
			...
			if (this.document.getVersions() != null && !this.document.getVersions().isEmpty()) {
				...
				for (Iterator iter = this.document.getVersions().iterator(); iter.hasNext();) {
					...
					if(version.getUploadDocument()!=null) {
						uploadedFiles.put(version.getVersion(), version.getUploadDocument());
					}
				}				
			}
	 ...
	 * @param uploadedFiles 
	 */
	public String doAfterSave(String result, Map<String, UploadedFile> uploadedFiles) {
		...
		if (this.document.getVersions() != null && !this.document.getVersions().isEmpty()) {
			...
			for (Iterator iter = this.document.getVersions().iterator(); iter.hasNext();) {
				...
				if (version.getUploadDocument() != null || uploadedFiles.containsKey(version.getVersion())) {
					try {
						String documentPath = this.document.getId() + File.separator
								+ version.getId();
						
						UploadedFile uploaded = (version.getUploadDocument()==null)?uploadedFiles.get(version.getVersion()):version.getUploadDocument();
						uploader.replace(documentPath, version.getOldDocumentPath(), uploaded);
						version.setDocumentPath(this.document.getId() + "/" + version.getId() + "/"
