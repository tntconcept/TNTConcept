...
<html>
  ...
  <body>
    ...
    <!-- settings.jsp: generated by stajanov code generator -->
    <f:loadBundle basename="com.autentia.tnt.resources.messages" var="msg" />
    <i:location place="settings" msg="${msg}"/> 
    ...
    <f:view>
     <%@include file="/inc/header.jsp" %>
      <h:form id="settings">
        ...
        <%-- Header --%>
        <i:titleBar msg="${msg}">
          <h:commandLink action="#{settingBean.search}">
            <h:graphicImage rendered="#{ settingBean.search.searchActive}" title="#{msg.entityActions_filtered}"  value="/img/search_applied.gif" styleClass="titleImg" />
            <h:graphicImage rendered="#{!settingBean.search.searchActive}" title="#{msg.entityActions_search}"    value="/img/search.gif"         styleClass="titleImg" />
          </h:commandLink>
          <h:commandLink action="#{settingBean.create}" rendered="#{settingBean.createAvailable}">
          ...
          <div class="abcPager">
            ...
            <a:abcPager styleClass="abcPagerLetter" selectedStyleClass="abcPagerSelectedLetter" 
                        value="#{settingBean.letter}" action="#{settingBean.letterClicked}"
                        allowUnselect="true" unselectImage="/img/no_funnel.png"/>
          </div>

        
        <%-- List of settings --%>
        <t:dataTable id="list" var="setting" value="#{settingBean.all}" preserveDataModel="false" 
            ...
            columnClasses="listCmdCell,listSettingName,listSettingValue,listSettingType"
            sortColumn="#{settingBean.sortColumn}" sortAscending="#{settingBean.sortAscending}"
	    ...
	    rowOnMouseOut="this.className=this.savedClassName;">
          ...
          <h:column>
            ...
            <t:commandLink action="#{settingBean.detail}" immediate="true">
              <f:param name="rowid" value="#{setting.id}" />
    ...
    <h:column>
      <f:facet name="header">
          ...
          <f:verbatim>${msg['setting.name']}</f:verbatim>
        </t:commandSortHeader>
      </f:facet>
    ...
    <h:column>
      <f:facet name="header">
      	        <t:commandSortHeader styleClass="listHeader" columnName="value">
                 <f:facet name="ascending">
            <t:graphicImage value="/img/ascending-arrow.gif" border="0" />
          </f:facet>
          <f:facet name="descending">
          ...
          <f:verbatim>${msg['setting.value']}</f:verbatim>
        </t:commandSortHeader>
      </f:facet>
