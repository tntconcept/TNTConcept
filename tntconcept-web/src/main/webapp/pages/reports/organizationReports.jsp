...
<html>
  ...
  <body>
    ...
    <i:location place="organizationReports" msg="${msg}" />
    
    <f:view>
      <%@include file="/inc/header.jsp" %>
      <c:if test="${organizationReportBean.launch}">
            <script>
				openReportParameters("organization/${organizationReportBean.selectedReport}.${organizationReportBean.format}","${organizationReportBean.parameters}","${organizationReportBean.selectMany}");
        	</script>
      </c:if>
      <%-- Report arguments user interface --%>
      <h:form id="organizationReports">
        
        <%-- Header --%>
        <i:titleBar msg="${msg}">
          <h:commandLink action="#{organizationReportBean.run}">
        ...
        <h:panelGrid columns="2" cellpadding="0" cellspacing="0" styleClass="editTable" columnClasses="editLabelRW,editFieldCell">
          ...
          <h:outputText value="#{msg['organizationReports.report']}:" />
          <h:selectOneMenu id="organizationReport" value="#{organizationReportBean.selectedReport}"
                           required="true" immediate="true" valueChangeListener="#{organizationReportBean.selectedReportChanged}" onchange="submit();">
            <f:selectItems value="#{organizationReportBean.reports}" />
          </h:selectOneMenu>
          
          <h:outputText value="#{msg['organizationReports.format']}:" />
          <h:selectOneMenu id="reportFormat" value="#{organizationReportBean.format}" required="true">
            <f:selectItems value="#{organizationReportBean.formats}" />
          </h:selectOneMenu>
        ...
        <h:dataTable id="params" var="reportParameter"
                     value="#{organizationReportBean.reportParametersDefinitions}"
