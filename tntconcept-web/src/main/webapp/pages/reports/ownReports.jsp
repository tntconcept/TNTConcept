...
<html>
  ...
  <body>
    ...
    <i:location place="ownReports" msg="${msg}" />
    
    <f:view>
      <%@include file="/inc/header.jsp" %>
      <c:if test="${ownReportBean.launch}">
            <script>
				openReportParameters("activity/${ownReportBean.selectedReport}.${ownReportBean.format}","${ownReportBean.parameters}","${ownReportBean.selectMany}");
        	</script>
      </c:if>
      <%-- Report arguments user interface --%>
      <h:form id="ownReports">
        
        <%-- Header --%>
        <i:titleBar msg="${msg}">
          <h:commandLink action="#{ownReportBean.run}">
        ...
        <h:panelGrid columns="2" cellpadding="0" cellspacing="0" styleClass="editTable" columnClasses="editLabelRW,editFieldCell">
          ...
          <h:selectOneMenu id="activityReport" value="#{ownReportBean.selectedReport}"
                           required="true" immediate="true" valueChangeListener="#{ownReportBean.selectedReportChanged}" onchange="submit();">
            <f:selectItems value="#{ownReportBean.reports}" />
          </h:selectOneMenu>
          
          <h:outputText value="#{msg['bitacoreReports.format']}:" />
          <h:selectOneMenu id="reportFormat" value="#{ownReportBean.format}" required="true">
            <f:selectItems value="#{ownReportBean.formats}" />
          </h:selectOneMenu>
        ...
        <h:dataTable id="params" var="reportParameter"
                     value="#{ownReportBean.reportParametersDefinitions}"
            ...
            <h:panelGroup id="selectOneSelectManys" rendered="#{reportParameter.selectOneSelectManyType == true}">
             <h:message styleClass="error" showSummary="true" showDetail="false" for="selectMany2" />
              <h:selectOneMenu id="selectOneOrg" onchange="submit()" immediate="true"
                 value="#{ownReportBean.selectedOrganization}" required="true" 
                 valueChangeListener="#{ownReportBean.selectedOrganizationChanged}">
                <f:selectItems value="#{reportParameter.items}" />                
              </h:selectOneMenu>
