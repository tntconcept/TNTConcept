...
<html>
  ...
  <body>
    ...
    <i:location place="billReports" msg="${msg}" />
    
    <f:view>
      <%@include file="/inc/header.jsp" %>
      <c:if test="${billReportBean.launch}">
            <script>
				openReportParameters("bill/${billReportBean.selectedReport}.${billReportBean.format}","${billReportBean.parameters}","${billReportBean.selectMany}");
        	</script>
      </c:if>
      <%-- Report arguments user interface --%>
      <h:form id="billReports">
        
        <%-- Header --%>
        <i:titleBar msg="${msg}">
          <h:commandLink action="#{billReportBean.run}">
        ...
        <h:panelGrid columns="2" cellpadding="0" cellspacing="0" styleClass="editTable" columnClasses="editLabelRW,editFieldCell">
          ...
          <h:outputText value="#{msg['billReports.report']}:" />
          <h:selectOneMenu id="billReport" value="#{billReportBean.selectedReport}"
                           required="true" immediate="true" valueChangeListener="#{billReportBean.selectedReportChanged}" onchange="submit();">
            <f:selectItems value="#{billReportBean.reports}" />
          </h:selectOneMenu>
          
          <h:outputText value="#{msg['billReports.format']}:" />
          <h:selectOneMenu id="reportFormat" value="#{billReportBean.format}" required="true">
            <f:selectItems value="#{billReportBean.formats}" />
          </h:selectOneMenu>
        ...
        <h:dataTable id="params" var="reportParameter"
                     value="#{billReportBean.reportParametersDefinitions}"
          ...
          <h:column id="data">
            ...
            <h:panelGroup id="years" rendered="#{reportParameter.yearType == true}">
              <h:selectOneMenu id="year" value="#{reportParameter.value}" required="true">
