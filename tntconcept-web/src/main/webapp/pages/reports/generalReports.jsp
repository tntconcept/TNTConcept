...
<html>
  ...
  <body>
    ...
    <i:location place="generalReports" msg="${msg}" />
      
    
    <f:view>
      <%@include file="/inc/header.jsp" %>
     <c:if test="${generalReportBean.launch}">
            <script>
				 openReportParameters("general/${generalReportBean.selectedReport}.${generalReportBean.format}","${generalReportBean.parameters}","${organizationReportBean.selectMany}");
        	</script>
      </c:if>
      <%-- Report arguments user interface --%>
      <h:form id="generalReports">
        
        <%-- Header --%>
        <i:titleBar msg="${msg}">
          <h:commandLink action="#{generalReportBean.run}">
        ...
        <h:panelGrid columns="2" cellpadding="0" cellspacing="0" styleClass="editTable" columnClasses="editLabelRW,editFieldCell">
          ...
          <h:outputText value="#{msg['generalReports.report']}:" />
          <h:selectOneMenu id="generalReport" value="#{generalReportBean.selectedReport}"
                           required="true" immediate="true" valueChangeListener="#{generalReportBean.selectedReportChanged}" onchange="submit();">
            <f:selectItems value="#{generalReportBean.reports}" />
          </h:selectOneMenu>
          
          <h:outputText value="#{msg['generalReports.format']}:" />
          <h:selectOneMenu id="reportFormat" value="#{generalReportBean.format}" required="true">
            <f:selectItems value="#{generalReportBean.formats}" />
          </h:selectOneMenu>
        ...
        <h:dataTable id="params" var="reportParameter"
                     value="#{generalReportBean.reportParametersDefinitions}"
