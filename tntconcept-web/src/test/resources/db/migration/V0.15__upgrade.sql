...
alter table User ALTER column dayDuration integer(11);

-- Agregar horas máximas a la tabla de convenio
alter table WorkingAgreement add column yearDuration integer(11) not null default 0 ;
...
CREATE TABLE  CommissioningChange (
  ...
  PRIMARY KEY (id),
  KEY fk_commissioningChange_commissioning on (commissioningId),
  KEY fk_commissioningChange_user on (userId),
...
CREATE TABLE  CommissioningFile (
  ...
  PRIMARY KEY (id),
  KEY fk_commissioningFile_commissioning on (commissioningId),
  KEY fk_commissioningFile_user on (userId),
...
CREATE TABLE  ActivityFile (
  ...
  fileMime varchar(128)  DEFAULT NULL,
  PRIMARY KEY (id),
  KEY fk_activityFile_externalActivity on (externalActivityId),
  KEY fk_activityFile_user on (userId),
...
alter table Bill alter column name varchar(4096) ;
alter table Bill alter column name set not null;
alter table Bill alter column observations varchar(4096);

-- Aumentar el tamaño del concepto en el desglose de factura (BillBreakdown)
alter table BillBreakDown alter column concept varchar(4096) ;
alter table BillBreakDown alter column concept set not null;
