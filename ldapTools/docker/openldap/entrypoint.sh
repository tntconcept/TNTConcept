...
EOF
    ...
    slapadd -n1 -l ldif/initial.ldif

    slapcat -n0 -F /etc/ldap/slapd.d -l /tmp/config.ldif
    sed -i "s/olcAccess: {2}to \* by \* read/olcAccess: {4}to \* by \* read/g" /tmp/config.ldif
    sed -i "s/olcAccess: {1}to attrs=shadowLastChange/olcAccess: {2}to dn.one=\"ou=People,dc=autentia,dc=com\" attrs=pwdReset by self read by dn.exact=\"cn=reset,dc=autentia,dc=com\" write\nolcAccess: {3}to attrs=shadowLastChange/g" /tmp/config.ldif
    sed -i "s/olcAccess: {0}to attrs=userPassword by self write/olcAccess: {0} to dn.one="ou=People,dc=autentia,dc=com" attrs=userPassword by self write by dn.exact="cn=reset,dc=autentia,dc=com" write by anonymous auth by \* none\nolcAccess: {1}to attrs=userPassword by self write/g" /tmp/config.ldif
    rm -rf /etc/ldap/slapd.d/*
    slapadd -n0 -F /etc/ldap/slapd.d -l /tmp/config.ldif
    rm /tmp/config.ldif
